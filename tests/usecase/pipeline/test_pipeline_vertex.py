"""
PipelineUseCase の vertex_train / update_source_dataset / push_notebook 対応テスト。

なぜこのテストが必要か:
  - Pipeline が vertex_train / update_source_dataset / push_notebook ステップを
    正しく実行できることを保証する。
...
def _make_pipeline_cfg(steps: list[dict]) -> DictConfig:
    return OmegaConf.create(
        {
            "usecase": "pipeline",
            "job_id": "test_vertex_pipeline",
            "steps": steps,
        }
    )


class TestPipelineVertexTrainStep:
    def test_vertex_train_step_is_executed(self) -> None:
        """vertex_train ステップが実行されること。"""
        call_log: list[str] = []

        cfg = _make_pipeline_cfg([{"usecase": "vertex_train", "recipe": "lgbm"}])
        usecase = PipelineUseCase(
            ...
            run_vertex_train=lambda c: call_log.append("vertex_train"),
    ...
    def test_full_vertex_to_kaggle_pipeline(self) -> None:
        ...
        cfg = _make_pipeline_cfg(
            [
                ...
                {"usecase": "vertex_train", "recipe": "lgbm"},
    ...
    def test_unknown_step_raises_value_error_with_new_runners(self) -> None:
        ...
        usecase = PipelineUseCase(
            ...
            run_vertex_train=lambda c: None,
