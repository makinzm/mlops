"""
CloudTrainUseCase の単体テスト。

なぜこのテストが必要か:
  - CloudTrainUseCase は ObjectStorageRepository と TrainingJobRepository を受け取り、
    コード + データのオブジェクトストレージアップロード → リモート学習ジョブ送信 → 完了待機 →
    モデルのオブジェクトストレージダウンロード の一連の流れを担う。
  - 各リポジトリの Mock を使い、クラウド呼び出しなしで UseCase ロジックをテストする。
  - .gitignore 配置、CloudTrainResult の正しい構造を保証する。
"""
...
from src.usecase.training.cloud_train import CloudTrainResult, CloudTrainUseCase
...
def _make_cfg(tmp_path: Path) -> DictConfig:
    """CloudTrainUseCase 用の DictConfig を生成する。"""
...
class TestCloudTrainUseCaseExecute:
    """CloudTrainUseCase.execute() のテスト。"""

    def test_returns_cloud_train_result(self, tmp_path: Path) -> None:
        """execute() が CloudTrainResult を返すこと。"""
        cfg = _make_cfg(tmp_path)
        usecase = CloudTrainUseCase(
            cfg=cfg,
        ...
        assert isinstance(result, CloudTrainResult)
    ...
    def test_result_contains_commit_hash(self, tmp_path: Path) -> None:
        """CloudTrainResult に commit_hash が記録されること。"""
        cfg = _make_cfg(tmp_path)
        usecase = CloudTrainUseCase(
            cfg=cfg,
            object_storage=_make_mock_gcs(),
    ...
    def test_result_contains_gcs_uris(self, tmp_path: Path) -> None:
        """CloudTrainResult にオブジェクトストレージ URI が記録されること。"""
        cfg = _make_cfg(tmp_path)
        usecase = CloudTrainUseCase(
            cfg=cfg,
            object_storage=_make_mock_gcs(),
