"""
JobTrainUseCase の単体テスト。

なぜこのテストが必要か:
  - JobTrainUseCase は ObjectStorageRepository と TrainingJobRepository を受け取り、
    コード + データのオブジェクトストレージアップロード → 学習ジョブ送信 → 完了待機 →
    モデルのオブジェクトストレージダウンロード の一連の流れを担う。
  - 各リポジトリの Mock を使い、クラウド呼び出しなしで UseCase ロジックをテストする。
  - .gitignore 配置、JobTrainResult の正しい構造を保証する。
"""
...
from src.usecase.training.job_train import JobTrainResult, JobTrainUseCase
...
def _make_cfg(tmp_path: Path) -> DictConfig:
    """JobTrainUseCase 用の DictConfig を生成する。"""
...
class TestJobTrainUseCaseExecute:
    """JobTrainUseCase.execute() のテスト。"""

    def test_returns_job_train_result(self, tmp_path: Path) -> None:
        """execute() が JobTrainResult を返すこと。"""
        cfg = _make_cfg(tmp_path)
        usecase = JobTrainUseCase(
            cfg=cfg,
        ...
        assert isinstance(result, JobTrainResult)
    ...
    def test_submits_job_with_correct_params(self, tmp_path: Path) -> None:
        """学習ジョブが正しいコンテナ URI とマシンタイプで送信されること。"""
        cfg = _make_cfg(tmp_path)
    ...
    def test_raises_on_job_failure(self, tmp_path: Path) -> None:
        """学習ジョブが FAILED の場合に RuntimeError が送出されること。"""
        cfg = _make_cfg(tmp_path)
        mock_vertex = _make_mock_vertex()
    ...
    def test_result_contains_commit_hash(self, tmp_path: Path) -> None:
        """JobTrainResult に commit_hash が記録されること。"""
        cfg = _make_cfg(tmp_path)
        usecase = JobTrainUseCase(
            cfg=cfg,
            object_storage=_make_mock_gcs(),
    ...
    def test_result_contains_gcs_uris(self, tmp_path: Path) -> None:
        """JobTrainResult にオブジェクトストレージ URI が記録されること。"""
        cfg = _make_cfg(tmp_path)
        usecase = JobTrainUseCase(
            cfg=cfg,
            object_storage=_make_mock_gcs(),
    ...
    def test_env_vars_contain_gcs_uris(self, tmp_path: Path) -> None:
        """学習ジョブの env_vars に GCS_DATA_URI と GCS_MODEL_URI が含まれること。"""
        cfg = _make_cfg(tmp_path)
        mock_vertex = _make_mock_vertex()
        usecase = JobTrainUseCase(
            cfg=cfg,
