...
from src.domain.model.trainer import FoldResult, Trainer, TrainResult


def _make_fold(
    fold_idx: int = 0,
    train_score: float = 0.91,
    valid_score: float = 0.85,
    metric: str = "auc",
    model_path: Path = Path("models/titanic/fold_0/model.txt"),
    oof_path: Path = Path("models/titanic/fold_0/oof_train.parquet"),
    error_analysis_path: Path = Path("models/titanic/fold_0/error_analysis.parquet"),
    feature_importance_path: Path | None = Path("models/titanic/fold_0/feature_importance.parquet"),
    n_train: int = 712,
    n_valid: int = 179,
...
def _make_result(
    job_id: str = "titanic_lgbm",
    timestamp: str = "20260315T120000",
    commit_hash: str = "abc1234",
    trainer_type: str = "lgbm",
    output_dir: Path = Path("models/titanic/titanic_lgbm/20260315T120000"),
    fold_results: list[FoldResult] | None = None,
    cv_mean_score: float = 0.85,
    cv_std_score: float = 0.01,
    metric: str = "auc",
    seed: int = 42,
    ...
    if fold_results is None:
        fold_results = [
            _make_fold(fold_idx=0, valid_score=0.86),
            _make_fold(fold_idx=1, valid_score=0.84),
        ]
    return TrainResult(
        job_id=job_id,
        timestamp=timestamp,
        ...
        metric=metric,
        seed=seed,
        trainer_fallback=trainer_fallback,
...
class TestFoldResult:
    ...
    def test_feature_importances_populated(self) -> None:
        ...
        result = _make_fold(feature_importances=importances)
    ...
    def test_feature_importance_path_can_be_none(self) -> None:
        ...
        result = _make_fold(feature_importance_path=None)
...
class TestTrainResult:
    ...
    def test_cv_scores_stored(self) -> None:
        result = _make_result(cv_mean_score=0.85, cv_std_score=0.01)
    ...
    def test_trainer_fallback_fields(self) -> None:
        ...
        result = _make_result(trainer_fallback=True, trainer_requested="gpu_lgbm")
    ...
    def test_commit_hash_recorded(self) -> None:
        ...
        result = _make_result(commit_hash="deadbeef")
...
class TestTrainerProtocol:
    def test_protocol_is_runtime_checkable(self) -> None:
        ...
        assert hasattr(Trainer, "_is_protocol")
    ...
    def test_class_satisfying_protocol_passes_isinstance(self) -> None:
        ...
        class DummyTrainer:
            def fit_folds(
                ...
                fold = _make_fold()
                return _make_result(fold_results=[fold])
