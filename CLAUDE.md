...
- [ ] 調査をして、不明な内容があれば、必ずユーザーに質問すること。わからないまま作業を進めないこと。また、公式ドキュメントにない内容は実装しないこと。
- [ ] 計算についてはすべて、適切な計算量の表現がdocstringに記載されていること。特に、学習や推論のコードは、時間計算量と空間計算量の両方を記載すること。
- [ ] manual を記載する際は 必ずどの configの設定を変更して、どのようなコマンドを実行すればいいかを記載すること。コードの内容を説明するだけでは不十分で、ユーザーが実際に手を動かせるようにすること。
